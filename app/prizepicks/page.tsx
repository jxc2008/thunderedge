...
function buildResult(player: string, line: number) {
  const stats: StatCardData[] = [
    { label: 'Model Mean', value: (line + 2.1).toFixed(1), delta: 'kills / map', semantic: 'positive' },
    { label: 'PrizePicks Line', value: line.toFixed(1), delta: 'kill line', semantic: 'neutral' },
    { label: 'Sample Size', value: 44, delta: 'maps analyzed', semantic: 'neutral' },
    { label: 'K/Map Avg', value: (line + 1.6).toFixed(1), delta: 'last 20 maps', semantic: 'neutral' },
    { label: 'Edge', value: '+8.4pp', delta: 'vs market', semantic: 'positive' },
    { label: 'VLR Rating', value: '1.29', delta: 'overall rating', semantic: 'neutral' },
  ]
  const agents: AgentRow[] = [
    { agent: 'Jett', maps: 13, killsPerMap: line + 3.2, overPct: 69.2 },
    { agent: 'Neon', maps: 11, killsPerMap: line + 1.8, overPct: 54.5 },
    { agent: 'Raze', maps: 9, killsPerMap: line + 0.6, overPct: 44.4 },
    { agent: 'Reyna', maps: 7, killsPerMap: line - 0.9, overPct: 28.6 },
    { agent: 'Iso', maps: 4, killsPerMap: line - 2.1, overPct: 25.0 },
  ]
  const events: EventCardData[] = [
    {
      ...
      maps: [
        { map: 'Ascent', kills: Math.round(line + 2.5), line },
        { map: 'Bind', kills: Math.round(line + 5.5), line },
        { map: 'Haven', kills: Math.round(line - 1.5), line },
      ],
    },
    {
      ...
      maps: [
        { map: 'Pearl', kills: Math.round(line + 1.5), line },
        { map: 'Icebox', kills: Math.round(line - 2.5), line },
        { map: 'Lotus', kills: Math.round(line + 0.5), line },
      ],
    },
  ]
  const dist = Array.from({ length: 15 }, (_, i) => {
    ...
    return {
      kills: k,
      modelPct: parseFloat((Math.exp(-Math.pow(k - (line + 2.1), 2) / 18) * 18).toFixed(2)),
      marketPct: parseFloat((Math.exp(-Math.pow(k - line, 2) / 20) * 16).toFixed(2)),
    }
  })
...
const inputStyle: React.CSSProperties = {
  background: 'rgba(14,6,24,0.8)',
  border: '1px solid rgba(139,43,250,0.3)',
  color: '#ffffff',
    ...
    <>
      ...
      <div className="page-container" style={{ padding: '0 24px 3rem' }}>
        ...
        <div style={{ textAlign: 'center', padding: '3rem 0 2rem' }}>
          ...
          >
            Prize<span style={{ color: ACCENT }}>Picks</span> Analytics
          </h1>
          <p style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.5)', maxWidth: 600, margin: '0 auto 2rem' }}>
            Kill-line analytics tailored for PrizePicks — negative binomial modelling against VCT player props.
          </p>
        </div>
        ...
        <div
          style={{
            background: 'rgba(139,43,250,0.08)',
            border: '1px solid rgba(139,43,250,0.2)',
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'flex-end' }}>
            ...
            <button
              ...
              style={{
                ...
                background: isLoading || !playerInput.trim() ? 'rgba(139,43,250,0.4)' : ACCENT,
                color: '#ffffff',
                border: 'none',
                borderRadius: 0,
        ...
        {isLoading && (
          <div style={{ textAlign: 'center', padding: '3rem', color: '#71717a' }}>
            ...
            <p style={{ fontSize: '0.875rem' }}>Running model against PrizePicks line...</p>
        ...
        {!isLoading && result && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div
              style={{
                background: 'rgba(139,43,250,0.08)',
                border: '1px solid rgba(139,43,250,0.15)',
                borderLeft: `3px solid ${ACCENT}`,
            ...
            >
              {/* Left */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                <OverUnderDisplay overPct={62.4} underPct={37.6} sampleSize={44} killLine={result.line} />
                <RecommendationCard
                  type="BET_OVER"
                  ev={0.084}
                  confidence="HIGH"
                  reason={`Model mean ${(result.line + 2.1).toFixed(1)} exceeds PrizePicks line ${result.line.toFixed(1)}. Strong positive EV on more picks.`}
                />
