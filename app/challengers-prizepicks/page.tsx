...
const ACCENT = '#6D28D9'
const ACCENT_BG = 'rgba(109,40,217,0.08)'
  ...
  return (
    <div
      style={{
        ...
        borderLeft: `3px solid ${top3 ? ACCENT : isIncomplete ? 'rgba(255,255,255,0.08)' : 'rgba(109,40,217,0.25)'}`,
        marginBottom: '0.4rem',
...
// ─── Leaderboard tab (Challengers tier 2) ───────────────────────────────────
...
function LeaderboardTab({ onAddToParlay, onSwitchToParlay, onSwitchToHistory }: {
  ...
  async function handleRefresh() {
    setLoading(true); setProgress(10); setProgressMsg('Fetching Challengers leaderboard...')
    try {
      const r = await fetch(`/api/challengers/prizepicks/leaderboard?combo_maps=${comboMaps}`)
  ...
  function handleUpload(input: HTMLInputElement) {
    ...
    if (uploadCombo) params.set('combo_maps', uploadCombo)
    params.set('challengers', 'true')
    ...
    const interval = setInterval(() => {
      ...
      setProgressMsg((msg) => (msg.startsWith('Up') ? 'Parsing & ranking Challengers players...' : msg))
    }, 500)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', borderBottom: '1px solid rgba(255,255,255,0.06)', flexWrap: 'wrap' }}>
        <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1rem', color: '#fff' }}>CHALLENGERS LEADERBOARD</span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', padding: '0.75rem 1rem' }}>
        ...
        {visible.length === 0 && !loading ? (
          <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
            ...
            <div style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.25)' }}>
              {rows.length === 0 ? 'Load a leaderboard to see ranked Challengers players' : 'No players match the current filter'}
            </div>
          </div>
        ) : (
          visible.map((row, i) => (
        ...
        <div style={{ marginTop: '1.25rem', padding: '1rem', background: ACCENT_BG, border: `2px dashed rgba(109,40,217,0.4)` }}>
...
// ─── Analyze tab (Challengers tier 2) ────────────────────────────────────────
...
function AnalyzeTab() {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      const endpoint = `/api/challengers/prizepicks/edge/${encodeURIComponent(ign)}?line=${line}&combo_maps=${comboMaps}${oddsQ}${teamQ}`
      ...
      <div style={{ background: ACCENT_BG, border: `1px solid ${ACCENT_BORDER}`, borderLeft: `3px solid ${ACCENT}`, padding: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'flex-end' }}>
          <div style={{ flex: '1 1 180px' }}>
            <label style={LABEL}>Challengers Player IGN</label>
            <input type="text" value={playerInput} onChange={(e) => setPlayerInput(e.target.value)} onKeyDown={kk} placeholder="s0m, Zombs, aproto..." style={INPUT} />
          </div>
      ...
      {!loading && edge && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div style={{ background: ACCENT_BG, border: `1px solid ${ACCENT_BORDER}`, borderLeft: `3px solid ${ACCENT}`, padding: '0.875rem 1.5rem', display: 'flex', flexWrap: 'wrap', gap: '1.5rem', fontSize: '0.875rem', color: 'rgba(255,255,255,0.5)' }}>
            ...
            <span style={{ color: ACCENT, fontWeight: 600 }}>Challengers</span>
  ...
  async function simulate() {
    ...
    try {
      const r = await fetch('/api/prizepicks/parlay', {
        ...
        body: JSON.stringify({ legs: validLegs, combo_maps: comboMaps, challengers: true }),
      })
      const d = await r.json()
      if (!r.ok || d.error) throw new Error(d.error || 'Failed')
      setResult(d)
  ...
  return (
    <div style={{ padding: '1rem', overflowY: 'auto' }}>
      <div style={{ padding: '0.75rem 0', marginBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.12em', color: 'rgba(255,255,255,0.4)', marginBottom: '0.25rem' }}>PARLAY HIT PROBABILITY (CHALLENGERS)</div>
      ...
      <h3 style={{ marginBottom: '1rem', color: '#fff', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
        Challengers PrizePicks Parlay Simulator (2–6 legs)
      </h3>
        ...
        {legs.map((leg, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: 'minmax(160px,1fr) 110px 110px auto', gap: '0.75rem', alignItems: 'end' }}>
            <div>
              ...
              <input type="text" value={leg.ign} onChange={(e) => updateLeg(i, 'ign', e.target.value)} placeholder="s0m" style={{ ...INPUT, width: '100%' }} />
            </div>
            <div>
...
function HistoryTab() {
  ...
  async function load() {
    setLoading(true)
    try {
      const r = await fetch('/api/prizepicks/leaderboard/history?limit=50&challengers=true')
      const d = await r.json()
  ...
  return (
    <div style={{ padding: '1rem', overflowY: 'auto' }}>
      <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '1rem', textTransform: 'uppercase', letterSpacing: '0.06em', color: '#fff', marginBottom: '1rem' }}>
        Challengers Leaderboard History
      </h3>
      ...
      {!loading && snapshots.length === 0 && <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.875rem' }}>No Challengers history available.</p>}
      {snapshots.map((s) => (
        <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 1rem', border: '1px solid rgba(255,255,255,0.06)', marginBottom: '0.5rem' }}>
          <div>
            ...
            <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)' }}>
              {(s.ranked_count ?? s.parsed_count ?? 0)} players · {s.source || 'challengers'}
            </div>
          </div>
          <a
